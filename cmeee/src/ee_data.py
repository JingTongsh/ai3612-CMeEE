...
class InputExample:
    ...
    def to_ner_task(self, for_nested_ner: bool = False):
        ...
        '''
        if self.entities is None:  # is_test
    ...
    def _preprocess(self, examples: List[InputExample], tokenizer) -> list:
        ...
        for example in tqdm(examples):
            ...
            if self.for_nested_ner:
                if not is_test:
                    label1, label2 = label
                    label = [[a, b] for a, b in zip(label1, label2)]
            ...
            for word, L in zip(text, label):
                ...
                if not is_test:
                    if not self.for_nested_ner:
                        ...
                        label1_ids.extend([label2id1[L[0]]] + [tokenizer.pad_token_id] * (len(token) - 1))
                        label2_ids.extend([label2id2[L[1]]] + [tokenizer.pad_token_id] * (len(token) - 1))
...
class CollateFnForEE:
    ...
    def __call__(self, batch) -> dict:
        ...
        labels2 = [x[2] for x in inputs] if len(inputs[0]) > 2 else None
...
if __name__ == '__main__':
    ...
    dataset = EEDataset(CBLUE_ROOT, mode="dev", max_length=30, tokenizer=tokenizer, for_nested_ner=True)

    batch = [dataset[0], dataset[212], dataset[4459]]

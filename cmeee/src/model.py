...
class CRFClassifier(nn.Module):
    ...
    def _pred_labels(self, emissions, attention_mask):
        '''NOTE: This is where to modify for CRF.
        ...
        '''
        pred_labels = self.crf.decode(emissions, attention_mask)
        for pred in pred_labels:
            pred += [0] * (self.seq_len - len(pred))

        pred_labels = torch.tensor(pred_labels, device=emissions.device)
    ...
    def forward(self, hidden_states, attention_mask, labels=None, no_decode=False, label_pad_token_id=NER_PAD_ID):
        ...
        if labels is None:
            pred_labels = self._pred_labels(emissions, attention_mask)
        else:
            loss = -self.crf.forward(emissions, labels, attention_mask, 'sum')
            if not no_decode:
                pred_labels = self._pred_labels(emissions, attention_mask)

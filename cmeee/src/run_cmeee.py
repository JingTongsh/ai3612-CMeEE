...
def main(_args: List[str] = None):
    ...
    if train_args.do_predict:
        ...
        # checkpoint = torch.load(join(train_args.output_dir, 'checkpoint-1000/pytorch_model.bin'))
        # model.load_state_dict(checkpoint['model_state_dict'])
        # model.eval()

        # np.ndarray, None, None
        predictions, _labels, _metrics = trainer.predict(test_dataset, metric_key_prefix="predict")
        np.save(f'../pred_{model_args.head_type}.npy', predictions)
        # predictions = np.load(f'../pred_{model_args.head_type}.npy')
